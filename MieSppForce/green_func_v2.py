...
def integrator(f, key, limit=4000):
    cache = load_cache()
    if key in cache:
        return cache[key]
...
def precompute_integrals(wl, h, r, forH, eps_val):
    ...
    # if forH:
    #     def rp(kr): return 0
    #     def rs(kr): return frenel.reflection_coeff(wl, lambda _: eps_val, kr)[0]
    # else:
    #     def rp(kr): return frenel.reflection_coeff(wl, lambda _: eps_val, kr)[0]
    #     def rs(kr): return 0
    
    # def rs(kr): return kr*0
